...
part_table_sql="""with t1 as
 (select segment_name, partition_name,owner
    from (select bytes / 1024 / 1024 part_size, segment_name, partition_name,owner
            from dba_segments
           where segment_type = 'TABLE PARTITION' and owner=upper('%s')
           order by 1 desc, 2)
   where part_size > %d
     and part_size <= %d)
select table_name||'.'||t1.partition_name,no_where ||t1.owner||'.'||TABLE_NAME|| ' partition(' || t1.partition_name || ') t'
  from (select 'select /*+ parallel(t,%d) */ ' || listagg||' from  '  no_where,table_name
          from (select distinct table_name,
                       LISTAGG('\\"'||column_name||'\\"', ',') WITHIN GROUP(ORDER BY COLUMN_ID) LISTAGG
                  from (select distinct table_name,
                                        column_id,
                                        column_name as column_name
                          from dba_tab_columns
                         where table_name in (select distinct segment_name from t1) and owner=upper('%s')
                           and table_name %s in (SELECT distinct table_name FROM dba_indexes where uniqueness='UNIQUE' and owner=upper('%s'))
                         order by 2 desc)
                 group by table_name)) t2,t1
                 where t1.segment_name=t2.table_name"""
#子分区表sql模板
subpart_table_sql="""with t1 as
 (select segment_name, partition_name,owner
    from (
    select subt.partition_name,
    seg.segment_name,
    seg.owner,
    sum(seg.bytes)/1024/1024 as part_size
    from DBA_TAB_SUBPARTITIONS subt ,dba_segments seg
    where subt.table_owner='%s'
    and seg.partition_name=subt.subpartition_name
    and seg.owner=subt.table_owner
    and seg.segment_name=subt.table_name
    group by subt.partition_name,
    seg.segment_name,
    seg.owner)
   where part_size > %d
     and part_size <= %d)
...
(select segment_name,owner
    ...
    from dba_segments
        where segment_type = 'TABLE' and owner=upper('%s')
    order by 1 desc, 2)
   where table_size > %d
     and table_size <= %d)
...
normal_table_sql_pk = normal_table_sql % (owner,0,10240000,1,owner,'',owner)
normal_table_sql_nopk = normal_table_sql % (owner,0,10240000,1,owner,'not',owner)

part_table_no_parallel_pk = part_table_sql % (owner,0,1024,1,owner,'',owner)
part_table_no_parallel_nopk = part_table_sql % (owner,0,1024,1,owner,'not',owner)
part_table_two_parallel_pk = part_table_sql % (owner,1024,10240000,2,owner,'',owner)
part_table_two_parallel_nopk = part_table_sql % (owner,1024,10240000,2,owner,'not',owner)

subpart_table_no_parallel_pk = subpart_table_sql % (owner,0,1024,1,owner,'',owner)
subpart_table_no_parallel_nopk = subpart_table_sql % (owner,0,1024,1,owner,'not',owner)
subpart_table_two_parallel_pk = subpart_table_sql % (owner,1024,10240000,2,owner,'',owner)
subpart_table_two_parallel_nopk = subpart_table_sql % (owner,1024,10240000,2,owner,'not',owner)

#part_table_four_parallel_p = part_table_sql % (owner,10240,20480,4,owner,'',owner)
#part_table_four_parallel_nop = part_table_sql % (owner,10240,20480,4,owner,'not',owner)
#part_table_six_parallel_p = part_table_sql % (owner,20480,10240000,6,owner,'',owner)
#part_table_six_parallel_nop = part_table_sql % (owner,20480,10240000,6,owner,'not',owner)
...
def get_table_column_dict(oracle_config):
    ...
    oracleCursor = oracleConn.cursor()
    exec_sql="""
        select TABLE_NAME ,LISTAGG('"`' || column_name || '`"', ',') WITHIN GROUP(ORDER BY COLUMN_ID) as column_name
          from dba_tab_columns where owner=upper('%s') group by TABLE_NAME
    """ % owner
...
def insert_job(mysql_connect,table_name,owner,job_name):
  ...
  try:
    cur = conn.cursor()
    cur.execute("delete from sync_log.sync_log where table_name='%s' and schema_name='%s'" % (table_name,owner))
...
def write_normal_table_job(oracle_connect,mysql_connect,sql_statment,owner,jobconfig):
    ...
    for normal_table_name in table_part_nosplit_tbname:
        ...
        job=jobconfig.replace("qSQL",query_sql).replace("tableName",normal_table_name).replace("ColumnInfo",column_info).replace("WriteUrl",write_url).replace("ReadUrl",read_url).replace("ReadUsername",read_username).replace("ReadPassword",read_password).replace("WriteUsername",write_username).replace("WritePassword",write_password).replace("RealChannel",real_channel)
...
if __name__ == "__main__":
    ...
    write_normal_table_job(oracle_config,mysql_config,normal_table_sql_pk, tdb,jobCnf)
    write_normal_table_job(oracle_config,mysql_config,normal_table_sql_nopk,tdb,jobCnf)
    # write part table job config
    write_part_table_job(oracle_config,mysql_config,part_table_no_parallel_pk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,part_table_no_parallel_nopk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,part_table_two_parallel_pk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,part_table_two_parallel_nopk,tdb,jobCnf)
    # write subpart table job config
    write_part_table_job(oracle_config,mysql_config,subpart_table_no_parallel_pk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,subpart_table_no_parallel_nopk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,subpart_table_two_parallel_pk,tdb,jobCnf)
    write_part_table_job(oracle_config,mysql_config,subpart_table_two_parallel_nopk,tdb,jobCnf)
