...
def get_source_count(oracle_connect,mysql_connect,source_schema,target_schema,table_name):
    ...
    if table_name.find('.') == -1 :
        tb_name = table_name
        count_sql = 'select count(*) from %s.%s' % (source_schema, tb_name)
    else:
    ...
    try:
        ...
        printf ('Failed execute select count(*) from %s.%s partition(%s)\n') % (source_schema,tb_name,part_name)
        printException (exception)
        sys.exit(1)
    ...
    update_source_sql = "update sync_log.sync_log set source_count=%d where table_name='%s' and schema_name='%s' " % (source_rows,table_name,target_schema)
    oracleCursor.close()
    oracleConn.close()
    ...
    try:
        ...
        print("Failed execute update sync_log.sync_log set source_count=%d where table_name='%s' and schema_name='%s' .\n") % (source_rows,table_name,target_schema)
...
def get_target_count(mysql_connect,target_schema,table_name):
    ...
    update_target_sql = "update sync_log.sync_log set target_count=%d where table_name='%s' and schema_name='%s' " % (target_rows,table_name,target_schema)
    ...
    try:
        ...
        print("Failed execute update sync_log.sync_log set target_count=%d where table_name='%s' and schema_name='%s' .\n") % (target_rows,table_name,target_schema)
        print "MySQL Error %d:%s" % (e.args[0], e.args[1])
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if only_validate_data == '1':
        ...
        for table_name in validate_table_dict:
            ...
            job1 = vd_pool.submit(get_source_count,oracle_config,mysql_config,suser,tdb,table_name)
            job2 = vd_pool.submit(get_target_count,mysql_config,tdb,table_name)
